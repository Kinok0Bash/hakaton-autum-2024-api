...
import com.uwu.authenticationservice.dto.request.AuthenticationRequest
import com.uwu.authenticationservice.dto.request.RegistrationRequest
import com.uwu.authenticationservice.dto.response.AuthenticationResponse
...
import com.uwu.authenticationservice.util.convertToMemberData
import com.uwu.authenticationservice.util.convertToUserDTO
...
class AuthenticationService(
    ...
    fun authorization(request: AuthenticationRequest, response: HttpServletResponse): AuthenticationResponse {
        ...
        val tokensEntity = RefreshTokensEntity (
            user = user,
            token = tokens[1]
        )
    ...
    fun refresh(userToken: String, response: HttpServletResponse): AuthenticationResponse {
        ...
        val user = userRepository.findByEmail(jwtService.extractUsername(userToken))

        val refreshTokens: MutableList<RefreshTokensEntity> = refreshTokensRepository.findAll()
        ...
        refreshTokensRepository.save(RefreshTokensEntity(
            user = user,
            token = tokens[1]
        ))
